...
class SQLBotClient:
    """
    Robust Client for DataEase SQLBot using Admin JWT (Session Token).
    """
    ...
    def generate_sql(self, question: str) -> Optional[str]:
        ...
        if not conf["key"]:
            print("❌ SQLBOT_API_KEY is empty")
            return None

        # Headers exactly matching your captured browser request
        headers = {
            "X-SQLBOT-TOKEN": conf["key"], # Bearer eyJ... goes here
            "Content-Type": "application/json",
            "Accept": "application/json"
        }

        try:
            # 1. Standard Chat Session Start
            ...
            print(f"📡 Requesting SQL from SQLBot (Admin Mode)...")
